      ...
      {posts.map((p) => {
        ...
        return (
          <Wrapper
            ...
            className={`grid grid-cols-1 sm:grid-cols-[130px_1fr] gap-2 sm:gap-6 border-t py-6 no-underline text-inherit ${p.url ? "cursor-pointer" : ""}`}
          ...
          >
            ...
            <div>
              ...
              {p.url && <span className="text-xs" style={{ fontFamily: "var(--font-mono)", color: "var(--accent)" }}>read →</span>}
            </div>
          </Wrapper>
        );
      })}
